...
<?php
    ...
    // Output as JSON for grid and list
    wp_send_json([
        'grid' => $html,
        'list' => $html, // Same HTML — styled differently in CSS
    ]);
} else {
    wp_send_json([
        'grid' => '<p class="jobs-msg">No jobs available.</p>',
        'list' => '<p class="jobs-msg">No jobs available.</p>',
    ]);
}
    wp_reset_postdata();
    wp_die();
